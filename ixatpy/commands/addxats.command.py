def addxatsCommand(args, client, server):
	if client.info["id"] in server.config["staff"]:
		...
		if len(uRow) == 1:
			try:
				float(args[2])
				server.database.query('update `users` set `xats` = `xats`+%(xats)s where `username` = %(username)s', {"xats": args[2], "username": args[1]})
				user = server.getUserByID(uRow[0]['id'], client.info['chat'])
				if user != False:
					user.send(server.doLogin(uRow[0]['username'], uRow[0]['password']))
				client.send('<m t="Successfuly added ' + str(args[2]) + ' xat(s) to ' + str(uRow[0]['username']) + '(' + str(uRow[0]['id']) + ')" u="0" />')
			except:
				pass
