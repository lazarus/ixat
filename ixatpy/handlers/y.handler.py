def yHandler(packet, client):
    from random import randint, shuffle
    from time import time
    from json import dumps

    if client.sentY != True and "r" in packet and "u" in packet:
        ...
        for value in yKeys.items():
            ...
            packetSend += '{attribute}="{value}" '.format(
                **{"attribute": value[1][0], "value": value[1][1]})
        packetSend += "/>"
        client.yPacket = packet
        client.send(packetSend)
        client.sentY = True
        client.mob = True if 'z' in packet and (packet['z'] == str(config.X_MOB_Z) or packet['z'] == "8335799305056508195") else False
    else:
        client.shutdown()
