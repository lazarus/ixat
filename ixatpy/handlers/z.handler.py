def zHandler(packet, client, server):
	import math
	...
	t2 = packet['t']
	try: t = t2[0:2]
	except ValueError: t = t2
	try: t3 = t2[3:]
	except ValueError: t3 = ''
	try: param = t2[2:]
	except ValueError: param = ''
		...
		if t[0:1] == "/":
			if t == '/l':
				...
				try:
					...
					if _user != False and (not _user.info['test'] or (client.info['chat'] == _user.info['chat'])):
						...
						_user.sendTo = client
						_user.send('<z d="' + str(_user.info['id']) + '" t="/l" u="' + str(client.info['id']) + '" />')
				except:
					return
			if t == '/a':
				...
				_user = client.sendTo
				s = False
				packet = '<z d="'+str(_user.info['id'])+'" u="'+str(client.info['id'])+'" '
				if client.hasPower(5, True) and _user.info['chat'] != client.info['chat'] and t2 == '/a_NF':
					s = '_NF'
				else:
					if t2[2:6] == "http":
						s = 'http://'+server.config['server_domain']+'/'+client.info['group']
					else:
				...
				if client.registered:
					...
					for po in client.loadedIndex:
						packet += 'p'+str(po)+'="'+str(client.loadedIndex[po])+'" '
					if len(client.userInfo['dO']) > 0:
						packet += 'po="'+str(client.userInfo['dO'])+'" '
					if client.hasPower(27, True):
						packet += 'x="'+str(client.userInfo['xats'])+'" y="'+str(math.floor(client.userInfo['days'] / 86400))+'" '
				packet += 'n=' + quoteattr(client.info['name']) + ' a=' + quoteattr(client.info['avatar']) + ' h=' + quoteattr(client.info['home']) + ' q="' + str(client.info["q"]) + '" v="2" />'
				_user.send(packet)
				client.sendTo = None
		else:
			if not 's' in packet:
			...
			for blocked in server.config["blockedixats"]:
				if blocked in str(packet['t']).lower().replace(" ", "") or "23.ip-5-196-225" in str(packet['t']).lower().replace(" ", ""):
					server.database.insert('advertisers', {'sender': int(client.info['id']), 'message': str(packet['t']), 'chatid': int(client.info['chat'])})
					client.sendRoom('<m t="I tried to advertise an ixat to an offline user." u="' + str(client.info['id']) + '" i="0" />')
			...
			if not offline:
				_user.send('<z u="'+str(client.info['id'])+'" t=' + quoteattr(packet['t']) + ' s="'+str(packet['s'])+'" d="'+str(_user.info['id'])+'" />')
			else:
